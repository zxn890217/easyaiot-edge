...
wire_runtime_override() {
  ...
  #   —— rkvdec/VEPU 平台匹配失败，开了 hwaccel=rkmpp 的 RUNTIME 直接 SIGSEGV 崩溃循环。
  # /proc/device-tree 是指向 /sys/firmware/devicetree/base 的绝对符号链接，MPP 在容器
  # namespace 里按这条路径解析。别指望「把 /sys/firmware/devicetree/base bind 进容器」：
  # docker 默认 maskPaths 含 /sys/firmware（runc 拿空的只读 tmpfs 盖住它，且在业务卷
  # 之后套），板上 docker 28.1.1 实测 —— .Mounts 与 mountinfo 里 bind 记录都在，
  # 容器内 ls 仍报 No such file，bind 整个被 mask 盖掉。
  # 正解是 privileged：runc 对 privileged 容器不套 maskPaths，容器里的 /sys 就是宿主
  # sysfs 本身，那条符号链接自然可达。
  # 只在「宿主有设备树（x86/ACPI 平台没有）且检测到 rkmpp 运行库」时才开启，
  # 其它部署形态（含 GPU/CUDA）不受影响。
  local privileged_line=""
  if [[ -n "$mpp_ld_path" && -r /sys/firmware/devicetree/base/compatible ]]; then
    privileged_line="    privileged: true"
    print_info "检测到 rkmpp + 设备树：VIDEO 容器已开 privileged（容器内 /proc/device-tree/compatible 可达，MPP 认得出 rk3588）"
  fi
