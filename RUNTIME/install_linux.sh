...
# shellcheck disable=SC1091
source "$ROOT/scripts/mpp_sdk.sh"
...
# Rockchip MPP（rkvdec 硬解 + VEPU 硬编）：与 resolve_rknn_cmake_args 同构。
#   off            -DRUNTIME_WITH_MPP=OFF（显式写死，避免复用旧 build 目录里的缓存值）
#   on/auto 且有 SDK -DRUNTIME_WITH_MPP=ON -DMPP_SDK_ROOT=<dir>
#   auto 且无 SDK   不下开关，编纯软解/软编版本
# 与 RKNN 不同的是：这里的 SDK 必须是 RUNTIME/.mpp-sdk 那份「已重定向 GLIBC_2.29」的产物，
# 直接指向宿主 /usr 会在 ld 阶段炸（见 RUNTIME/scripts/mpp_sdk.sh 开头的说明）。
# 结果放在全局数组 MPP_CMAKE_ARGS。
resolve_mpp_cmake_args() {
  MPP_CMAKE_ARGS=()
  local want sdk
  want="$(printf '%s' "${RUNTIME_WITH_MPP:-auto}" | tr '[:upper:]' '[:lower:]')"
  if [[ "$want" =~ ^(off|0|false|no)$ ]]; then
    MPP_CMAKE_ARGS+=(-DRUNTIME_WITH_MPP=OFF)
    return 0
  fi
  sdk="$(mpp_sdk_probe || true)"
  if [[ -n "$sdk" ]]; then
    MPP_CMAKE_ARGS+=(-DMPP_SDK_ROOT="$sdk")
  fi
  # host 编译模式下宿主的 glibc 就够新，可以直接吃 /usr/include/rockchip；
  # 但只有仓库内那份（或显式 MPP_SDK_ROOT）才保证能被容器里的 ld 接受。
  if [[ "$want" =~ ^(on|1|true|yes)$ ]]; then
    MPP_CMAKE_ARGS+=(-DRUNTIME_WITH_MPP=ON)
    print_info "强制启用 rkmpp 硬件编解码后端（SDK: ${sdk:-系统头文件}）"
  elif [[ -n "$sdk" ]] || mpp_header_found; then
    print_info "检测到 Rockchip MPP SDK (${sdk:-系统头文件})，启用 rkvdec/VEPU 硬件编解码后端"
    MPP_CMAKE_ARGS+=(-DRUNTIME_WITH_MPP=ON)
  fi
  return 0
}
...
build_runtime_in_docker() {
  ...
  # Rockchip MPP：同上，容器里得有 rk_mpi.h 和一份链得动的 librockchip_mpp。
  # 只认仓库内暂存的那份（RUNTIME/.mpp-sdk）—— 宿主 /usr/include 对容器无效，
  # 宿主 /usr/lib 那份还带着容器没有的 GLIBC_2.29 版本节点。
  local mpp_sdk mpp_want
  mpp_sdk="$(mpp_sdk_probe || true)"
  mpp_want="$(printf '%s' "${RUNTIME_WITH_MPP:-auto}" | tr '[:upper:]' '[:lower:]')"
  if [[ -n "$mpp_sdk" ]]; then
    case "$mpp_sdk" in
      "$REPO"/*)
        docker_opts+=(-e "MPP_SDK_ROOT=/src/${mpp_sdk#"$REPO"/}")
        ;;
      *)
        docker_opts+=(-v "$mpp_sdk:$mpp_sdk:ro" -e "MPP_SDK_ROOT=$mpp_sdk")
        ;;
    esac
    print_info "MPP SDK 已提供给构建容器: $mpp_sdk"
    # 头文件齐了不代表链得动：暂存那份若还挂着 GLIBC_2.29，ld 阶段照样失败
    local mpp_lib
    mpp_lib="$(mpp_runtime_lib || true)"
    if [[ -n "$mpp_lib" ]] && mpp_needs_glibc_patch "$mpp_lib"; then
      print_warning "$mpp_lib 仍要求 ${MPP_GLIBIC_FROM}：容器（glibc 2.28）链接期会报 version not found"
      print_warning "  在盒子上重跑 ./install_rk3588.sh mpp-setup，让它把版本节点重定向到 ${MPP_GLIBIC_TO}"
    fi
  elif [[ "$mpp_want" =~ ^(on|1|true|yes)$ ]]; then
    print_error "RUNTIME_WITH_MPP=on，但主机上没有可用的 MPP SDK（rockchip/rk_mpi.h + librockchip_mpp）"
    print_error "  在盒子上先做一次：仓库根 ./install_rk3588.sh mpp-setup"
    print_error "  它会把 /usr/include/rockchip 与 librockchip_mpp 暂存到 RUNTIME/.mpp-sdk，"
    print_error "  并把 GLIBC_2.29 的版本依赖重定向到容器（AlmaLinux 8.10 / glibc 2.28）有的 GLIBC_2.17"
    print_error "  只想先跑软解软编：RUNTIME_WITH_MPP=auto 重试"
    return 1
  elif [[ "$mpp_want" != "off" ]]; then
    print_warning "未找到 MPP SDK（rockchip/rk_mpi.h）：本次编出的 RUNTIME 无 rkmpp 后端，编解码会落在 CPU"
    print_warning "  补齐后重跑本命令即可启用硬解/硬编：仓库根 ./install_rk3588.sh mpp-setup"
  fi
  docker_opts+=(-e "RUNTIME_WITH_MPP=${RUNTIME_WITH_MPP:-auto}")
...
build_runtime_on_host() {
  ...
  # Rockchip MPP：本机（或 RUNTIME/.mpp-sdk）有 rk_mpi.h + librockchip_mpp 就顺带编出硬解/硬编后端
  resolve_mpp_cmake_args
  if [[ ${#MPP_CMAKE_ARGS[@]} -gt 0 ]]; then
    cmake_extra+=("${MPP_CMAKE_ARGS[@]}")
  fi
  if ! "$cmake_bin" "$ROOT" \
    -B "$build_dir" \
...
write_node_env() {
  ...
  # 示例任务配置里的编解码后端：装到了 mpp 运行库就写死 rkmpp，否则留给 auto 现场探测
  local example_hwaccel="auto"
  if compgen -G "${install_dir}/lib/librockchip_mpp.so*" >/dev/null 2>&1 \
    || compgen -G "${install_dir}/.mpp-sdk/lib/librockchip_mpp.so*" >/dev/null 2>&1; then
    example_hwaccel="rkmpp"
  fi
...
# RK3588：RUNTIME 直连 librockchip_mpp（链接期依赖，解析不到进程起不来）。
# .mpp-sdk/lib 是 install_rk3588.sh mpp-setup 打过 GLIBC 版本节点的暂存份。
for _mpp in "\${RUNTIME_ROOT}/.mpp-sdk/lib" "\${RUNTIME_ROOT}/mpp-lib" "\${RUNTIME_ROOT}/lib"; do
  if compgen -G "\${_mpp}/librockchip_mpp.so*" >/dev/null 2>&1; then
    case ":\${LD_LIBRARY_PATH}:" in
      *":\${_mpp}:"*) ;;
      *) export LD_LIBRARY_PATH="\${LD_LIBRARY_PATH}:\${_mpp}" ;;
    esac
  fi
done
export RUNTIME_PREFER_GPU="\${RUNTIME_PREFER_GPU:-true}"
...
force_cpu=false
gpu_device_id=0
# 编解码后端：auto | cuda | rkmpp | none（与 prefer_gpu / force_cpu 无关）
hwaccel=${example_hwaccel}
hwaccel_decode=true
hwaccel_encode=true
prefer_hwaccel=true
force_soft_av=false
